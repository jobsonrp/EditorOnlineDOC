-- phpMyAdmin SQL Dump
-- version 4.8.0.1
-- https://www.phpmyadmin.net/
--
-- Host: localhost
-- Generation Time: 12-Dez-2018 às 08:33
-- Versão do servidor: 10.1.32-MariaDB
-- PHP Version: 7.2.5

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
SET AUTOCOMMIT = 0;
START TRANSACTION;
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;

--
-- Database: `mywebdoc`
--
CREATE DATABASE IF NOT EXISTS `mywebdoc` DEFAULT CHARACTER SET latin1 COLLATE latin1_swedish_ci;
USE `mywebdoc`;

-- --------------------------------------------------------

--
-- Estrutura da tabela `arquivos`
--

CREATE TABLE `arquivos` (
  `id` int(11) NOT NULL,
  `nome` varchar(100) NOT NULL,
  `conteudo` longtext NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Extraindo dados da tabela `arquivos`
--

INSERT INTO `arquivos` (`id`, `nome`, `conteudo`) VALUES
(2, 'doc1', '<h1 style=\"text-align: center;\">Teste</h1><h1 style=\"text-align: center;\"><img src=\"data:image/jpeg;'),
(4, 'teste2', 'Teste 345<div>345 yyyyyyy</div><div>eee<br /></div><div>55567676tyty</div>'),
(5, '999', '545sdfdf<div>dfdf</div>'),
(6, '888', '<h1 style=\"text-align: center;\">Teste 64646</h1><div>eeeww</div><h1 style=\"text-align: center;\"><img src=\"data:image/jpeg;base64,\" alt=\"Resultado de imagem para image\" /></h1><p>12345 78999</p>'),
(7, '777', '\0'),
(8, '', 'Tetast<div>345</div><div><br /></div><div>45</div>');

--
-- Indexes for dumped tables
--

--
-- Indexes for table `arquivos`
--
ALTER TABLE `arquivos`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `nome` (`nome`);

--
-- AUTO_INCREMENT for dumped tables
--

--
-- AUTO_INCREMENT for table `arquivos`
--
ALTER TABLE `arquivos`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
COMMIT;

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
